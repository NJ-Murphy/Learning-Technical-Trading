classdef learning_IntradayDaily_PBO
    ...
    methods
        %% constructor
        function p = learning_IntradayDaily_PBO(varargin)
            % P = PATTERN(X,K,ELL,CI,NTYPE,LNTYPE,LTYPE) For data X (OHLCV)
            ...
            if nargin==0
               ...
               % p.B = nan(p.dimIntraday(3)+p.dimdaily(3)+1,noofportfoliostocks+1); % Time x Objects (assets + rf asset)   
                ...
                %p.expertparams = nan(noofexperts,4); %keeps trac of free parameters used
        ...
        function p = offline_Intraday(p)
            ...
            for t = p.tmin0:T   %day loop
               ...
               if p.ind_jump > 1 && t == p.ind_jump
                  ret_all_daily = [ret_all_daily;ones(size(Pc_daily,1)+1,1)'];
               else
        ...
        function p = online_Intraday(varargin)
            ...
            %
            % See Also: PATTERN/ONLINE, PATTERN/PARAMLEARN
        ...
        function p = online_Interday(varargin)
            ...
            %   data from times 1 to t where t = {1:T}.
            %
            % See Also: PATTERN/ONLINE, PATTERN/CONTROLS
