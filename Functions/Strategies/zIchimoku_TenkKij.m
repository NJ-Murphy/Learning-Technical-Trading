...
%varargin)
    ...
    sig = nan(size(f,2),m(1));
 for j = 1:m(1) 
    for i = pl(2)+pl(3)+1:m(3)
        if (f(1,i,j)>=f(2,i,j)) && (f(1,i-1,j)<f(2,i-1,j))%bullish cross
            if (f(1,i,j)&& f(2,i,j))> (f(4,i,j)&&f(5,i,j))
              sig(i,j) = +3;
            elseif f(4,i,j)<=(f(1,i,j)&& f(2,i,j))<=f(5,i,j) || f(5,i,j)<=(f(1,i,j)&& f(2,i,j))<=f(4,i,j)
              sig(i,j) = +2;
            elseif (f(1,i,j)&& f(2,i,j))< (f(4,i,j) && (f(1,i,j) && f(2,i,j))<f(5,i,j))
                sig(i,j) = +1;
            end
        elseif f(1,i,j)<=f(2,i,j) && f(1,i-1,j)>f(2,i-1,j) %bearish cross
            if (f(1,i,j)&& f(2,i,j))< (f(4,i,j) && f(5,i,j))
                sig(i,j) = -3;
            elseif f(4,i,j)<=(f(1,i,j)&& f(2,i,j))<=f(5,i,j) || f(5,i,j)<=(f(1,i,j)&& f(2,i,j))<=f(4,i,j)
                sig(i,j) = -2;
