function out = Ichimoku_KijSen(Data, n1, n2)
%varargin)
    ...
    n1 = Data{2};
    n2 = Data{3};
    m = size(Data{1}); %stocks x prices x time
    Data = Data{1};
   ...
   % Kijun Sen Cross
   sig = nan(m(3)-(pl(2)+pl(3)+1),m(1));
   for j = 1:m(1) 
    for i = (pl(2)+pl(3)+1):m(3) %time loop
        if (f(2,i,j)<=cl(i,j)) & (cl(i-1,j)<f(2,i-1,j))%bullish cross
            if (cl(i,j)>f(4,i,j)) & (cl(i,j)>f(5,i,j))
              sig(i,j) = +3;
            elseif f(4,i,j)<=cl(i,j) & cl(i,j)<=f(5,i,j)
              sig(i,j) = +2;
            elseif cl(i,j)< f(4,i,j) & cl(i,j)<f(5,i,j)
                sig(i,j) = +1;
            end
        elseif (f(2,i,j)>cl(i,j)) & (cl(i-1,j)>=f(2,i-1,j)) %bearish cross
            if (cl(i,j)<f(4,i,j)) & (cl(i,j)<f(5,i,j))
              sig(i,j) = -3;
            elseif f(4,i,:)<=cl(i,j) & cl(i,j)<=f(5,i,j)
              sig(i,j) = -2;
            elseif cl(i,j)> f(4,i,j) & cl(i,j)>f(5,i,j)
                sig(i,j) = -1;
            end
