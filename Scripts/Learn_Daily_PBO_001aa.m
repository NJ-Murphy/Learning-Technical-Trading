...
for i = 1:length(inds)-1
    ...
    p = learning_PBO(DataDaily_temp,k,ell,strats,STEFI,dayinterval,stocknames,sectors,noofportfoliostocks,liquidstocks,weighttype,ind_jump);

    % Implement the learning procedure by calling 'offline' function from the 'learning' class
    p1 = offline(p);

    M_pnl_daily(:,i) = p1.PnL;
    M_S_daily(:,i) = p1.S;
end

xlswrite(strcat('C:\Users\nicjm\Documents\R\Masters\M_Tim_daily_pnl','.xlsx'),M_pnl_daily) 
xlswrite(strcat('C:\Users\nicjm\Documents\R\Masters\M_Tim_daily_S','.xlsx'),M_S_daily) 
...
% % 
% % %     if i == 1
% % %      plot(Datetimevec,S_beststock(tmin0:end))
% % %      legend('Overall Portfolio Wealth','Best Stock','Location','Best');
% % %      plot(Datetimevec,S_uBAH(tmin0:end))
% % %      legend('Overall Portfolio Wealth','UBAH','Location','Best');
% % %     end
% end
% 
