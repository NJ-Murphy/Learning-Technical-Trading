classdef learning_daily
    ...
    methods
        %% constructor
        function p = learning_daily(varargin)
        ...
        function p = offline(p)
            ...
            for t = p.tmin0:T   %time loop
              Pc = reshape(p.x(:,1,t-1:t),size(p.x(:,1,t-1:t),1),size(p.x(:,1,t-1:t),3)); %closing prices time t-1 to t                ret_all = [ret_all; [transpose(Pc(:,end)./Pc(:,end-1)),p.STEFI(t,1)./p.STEFI(t-1,1)]];
